...
class PriceQuestionItem {
  constructor(public title: string, public subtitle: string) {}
}

const priceQuestionItems = [
  new PriceQuestionItem("howToTrailFree", "paymentMethodAnswer"),
  new PriceQuestionItem("paymentMethod", "paymentMethodAnswer"),
  new PriceQuestionItem("conIChangePlan", "paymentMethodAnswer"),
  new PriceQuestionItem("canICancel", "paymentMethodAnswer"),
];

export default function PriceQuestions() {
  ...
  return (
    ...
    >
      ...
      >
        <InnerContent>
          <Typography variant="h4">{pt("questionTitle")}</Typography>
          <Box sx={{ mt: 2 }}>
            {priceQuestionItems.map((item, index) => {
              return (
