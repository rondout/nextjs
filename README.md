...
- 使用 priority（boolean）属性：使用该属性后，页面会等待该 Image 标签对应的图片加载成功再展示，优点是不会造成页面骨架已经渲染，图片却还在加载的问题，缺点是会影响用户看到页面的时间，所以要谨慎使用。

#### 动态导入全局 CSS

在 Next 中，全局 css 只能从`_app.tsx`里面导入，但是我们有的第三方库的 css 文件并不想在每个 page 里面都导入，这种情况我们可以使用`import(...)`函数动态导入：
